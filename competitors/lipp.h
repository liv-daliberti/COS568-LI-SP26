...
template<class KeyType>
class HybridPGMLIPPAsyncLippAlias: public Lipp<KeyType>
{
public:
    explicit HybridPGMLIPPAsyncLippAlias(const std::vector<int>& params)
        : Lipp<KeyType>(params) {}

    std::string name() const { return "HybridPGMLIPPAsync"; }

    std::vector<std::string> variants() const {
        return {"stock_lipp_tu"};
    }
};

template<class KeyType>
class HybridPGMLIPPAsyncLippFast: public Base<KeyType>
{
public:
    explicit HybridPGMLIPPAsyncLippFast(const std::vector<int>& params) {
        (void) params;
    }
    ...
    inline __attribute__((always_inline)) size_t EqualityLookup(
        ...
        (void) thread_id;
        return static_cast<size_t>(lipp_.find_value_or_fast(
